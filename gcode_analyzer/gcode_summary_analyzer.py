...
from .segment_extractor import SlicerType, SlicerDetector
from .summary import build_layer_map
...
class GCodeComprehensiveSummary:
    ...
    def to_llm_summary(self) -> str:
        ...
        lines = [
            ...
            "",
            "--- 속도 설정 ---",
            f"출력 속도: {self.feed_rate.min_speed:.1f} ~ {self.feed_rate.max_speed:.1f} mm/s",
            f"평균 속도: {self.feed_rate.avg_speed:.1f} mm/s",
            f"이동 속도 평균: {self.feed_rate.travel_speed_avg:.1f} mm/s",
            f"출력 속도 평균: {self.feed_rate.print_speed_avg:.1f} mm/s",
            "",
            "--- 레이어 정보 ---",
...
class GCodeSummaryAnalyzer:
    ...
    def __init__(self, lines: List[GCodeLine], file_path: str = ""):
        ...
        # 레이어 매핑 생성 (다양한 슬라이서 지원)
        self._layer_map = build_layer_map(lines)
    ...
    def _analyze_basic_info(self):
        ...
        # 슬라이서 감지 (새로운 SlicerDetector 사용)
        slicer_type, slicer_version = SlicerDetector.detect(self.lines)

        # 슬라이서 이름 매핑
        slicer_names = {
            SlicerType.ORCASLICER: "OrcaSlicer",
            SlicerType.BAMBUSTUDIO: "Bambu Studio",
            SlicerType.CURA: "Ultimaker Cura",
        ...
        slicer_name = slicer_names.get(slicer_type, "Unknown")
        if slicer_version:
            self.summary.slicer_info = f"{slicer_name} {slicer_version}"
        else:
            self.summary.slicer_info = slicer_name

        # 필라멘트 타입 감지 (기존 로직 유지 + 확장)
    ...
    def _analyze_feed_rate(self):
        ...
        bucket_size = 50  # 50 mm/s 단위

        for line in self.lines:
            if line.cmd in ["G0", "G1"]:
                if "F" in line.params:
                    speed = line.params["F"]  # mm/min
                    speed_mms = speed / 60.0   # mm/s로 변환
                    all_speeds.append(speed)

                    # 버킷 분류 (mm/s 기준)
                    bucket = int(speed_mms // bucket_size) * bucket_size
                    bucket_key = f"{bucket}-{bucket + bucket_size}"
                    speed_buckets[bucket_key] += 1

                    # 이동 타입 분류
                    # G0는 항상 travel, G1은 E가 있으면 print, 없으면 travel
                    if line.cmd == "G0":
                        ...
                        if "E" in line.params:
                            ...
                            # G1 without E = travel (OrcaSlicer/BambuStudio 스타일)
                            travel_speeds.append(speed)

        # F 값은 mm/min 단위이므로 mm/s로 변환 (÷60)
        if all_speeds:
            feed_profile.all_speeds = [s / 60.0 for s in all_speeds]
            feed_profile.min_speed = min(all_speeds) / 60.0
            feed_profile.max_speed = max(all_speeds) / 60.0
            feed_profile.avg_speed = (sum(all_speeds) / len(all_speeds)) / 60.0
            feed_profile.speed_distribution = dict(speed_buckets)

        if travel_speeds:
            feed_profile.travel_speed_avg = (sum(travel_speeds) / len(travel_speeds)) / 60.0

        if print_speeds:
            feed_profile.print_speed_avg = (sum(print_speeds) / len(print_speeds)) / 60.0
    ...
    def _analyze_layers(self):
        ...
        # layer_map에서 최대 레이어 번호 추출
        layer_count = max(self._layer_map.values()) + 1 if self._layer_map else 0
    ...
    def _analyze_support(self):
        ...
        # 모델(비서포트) 키워드 - 다양한 슬라이서 지원
        model_keywords = [
            # Cura TYPE 스타일
            "TYPE:WALL", "TYPE:SKIN", "TYPE:FILL", "TYPE:PERIMETER",
            # OrcaSlicer/BambuStudio TYPE 스타일
            "TYPE:OUTER WALL", "TYPE:INNER WALL",
            "TYPE:SOLID INFILL", "TYPE:INTERNAL SOLID INFILL",
            "TYPE:SPARSE INFILL", "TYPE:INTERNAL INFILL",
            "TYPE:TOP SURFACE", "TYPE:BOTTOM SURFACE",
            "TYPE:BRIDGE", "TYPE:OVERHANG",
            "TYPE:SKIRT", "TYPE:BRIM", "TYPE:PRIME TOWER",
            # PrusaSlicer 스타일
            "TYPE:EXTERNAL PERIMETER", "TYPE:PERIMETER",
            "TYPE:TOP SOLID INFILL", "TYPE:BRIDGE INFILL",
            # 공통
            "TYPE:CUSTOM",
            # BambuStudio/OrcaSlicer FEATURE 스타일
        ...
        for idx, line in enumerate(self.lines):
            # layer_map에서 현재 레이어 가져오기
            current_layer = self._layer_map.get(idx, 0)

            # 서포트 구간 감지 (다양한 슬라이서 포맷)
            if line.comment:
                comment = line.comment.strip().upper()
                if any(kw in comment for kw in support_keywords):
                    in_support = True
                elif any(kw in comment for kw in model_keywords):
                    in_support = False
    ...
    def _analyze_sections(self):
        """구간 분석 (START/BODY/END) - layer_map 사용"""
        start_end = 0
        body_end = len(self.lines)

        # START 끝 찾기: layer_map에서 레이어 0 시작점 찾기
        for idx in sorted(self._layer_map.keys()):
            if self._layer_map[idx] == 0 and idx > 50:
                # 레이어 0이 시작되는 첫 번째 유의미한 위치
                start_end = idx
                break

        # 폴백: layer_map에서 못 찾으면 기존 로직
        if start_end == 0:
